...
class Player(object):
    ...
    def move(self):
        ...
        if self.position + roll <=99:
            ...
            print("You have to roll "+str(99-self.position)+" to land exactly on 100")
    def win(self):
        if self.position == 99:
            return self.token
        else:
...
class space(object):
    def __init__(self,move):
        self.move = move
    def movePlayer(self,player,board):
        oldPos = player.position
        player.position = player.position + self.move
        if self.move>0:
            print("This space is a ladder "+player.piece+" moves up",self.move,"spaces")
            input("[Enter to climb ladder]")
        elif self.move<0:
            print("This space is a chute "+player.piece+" moves down",self.move,"spaces")
            input("[Enter to slide down the chute]")
...
######
def main():
    numPlayers = 4 #askNum("How many players will be playing",2,4)
    players = []
    turn = 0
    winner = None
    board = Board()
    ...
    while not winner:
        print(players[turn].name +" peice "+ players[turn].token+" it is your turn.")
        input("[Enter to roll]")
        players[turn].move()
        playpos=players[turn].position
        space = board.board[playpos]
        space.movePlayer(players[turn],board)
        winner = players[turn].win()
        if not winner:
            turn = turn+1 #switchTurn(numPlayers,turn)
